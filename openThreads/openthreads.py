...
#TODO - rewrite the following line to become a dictionary that parses the monthly log and create indiviudal dictionaries of all pertinant header info for each e-mail and includes the content.
    ...
    def threader(self, message):
        """This function takes a set of messages and adds them to the threads tree system."""
        node_added = False
        if message['Reply'] == []:
            new_thread = thread(message):
            if not self.threads[new_thread.ID]:
                self.threads[thread.ID] = [new_thread]
                node_added = True
                break
        else:
            for thrd in self.threads:
                if message['Reply'] == thrd:
                    self.threads[thrd].add_node(message)
                    node_added = True
            if not node_added:
                for thrd in self.threads:
                    for msg in self.threads[thrd][messages]:
                        if message['Reply'] == msg:
                            self.threads[thrd].add_node(message, msg)
                            node_added = True
        if not node_added:
            raise Exception, "Message is non standard. Please copy and send to dev team to incorporate into future work \n" + self.cutHere + message + self.endCut
    ...
    class thread:
        ...
        def add_node(self, message, parent=None):
            """Add's a new thread object to the thread and will recusively search for a parent node if given a nested thread object"""
            if parent:
                branch = self.search_thread(parent)
                branch.add_node(message)
            else:
                node = thread(message)
                node.parent = self.ID
                
        def search_thread(self, parent):
            """Will search through the thread for a node and return it"""
